...
$defaults = array(
	"stwa_animation" => "animate__animated animate__bounce",
	"stwa_cookie" => "3",
...
$animation = preg_split("/[\s,]+/", $options['stwa_animation']);
...
$script = 'jQuery(document).ready(function($) {	
				...
				$(window).scroll(function () {
					...
					if(st > lastScrollTop){
					    if ( ( $( window ).scrollTop() + $(window).height() >= $(document).height() -' . $scroll_height . ') 
					    	...
					    	$(".stwa_arrow").hide();				    	
					    	$stwa_box.removeClass("animate__animated animate__' . $animation[1] . '");				    	
					    	$stwa_box.show();		    	
					    	$stwa_box.addClass("animate__animated animate__' . $animation[0] . '");
					    } 				    
					}
				...
				$(".stwa-close").on("click", function(){					
					$stwa_box.removeClass("animate__animated animate__' . $animation[0] . '");
			    	$stwa_box.addClass("animate__animated animate__' . $animation[1] . '");
			    	$(".stwa_arrow").show();
			    	$.cookie("stwa_scrollpopup", "true", { expires:' . $cookie_expire . ', path: "/" });
				});
